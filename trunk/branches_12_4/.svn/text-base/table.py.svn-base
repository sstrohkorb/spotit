...
class Table(object):
	...
	def make_table_initial(self, points):
		...
		card1 = Card(self.screen, 600, card_image_path)
		...
		card_list = card1.initial_card(points)
		return card_list
	...
	def make_new_table(self, int_list, points_new, points_old):
		...
		card1 = Card(self.screen, 600, card_image_path)
		card2 = Card(self.screen, 600, card_image_path)

		#insert the cards
        # This section has duplicated code, abstract it out. --KOH
		...
		int_list = card1.put_images_on_card(int_list, points_old, points_new)
